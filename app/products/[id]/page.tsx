...
export default function ProductDetailPage() {
  ...
  const handleAddToCart = async () => {
    if (product?.sizes && product.sizes.length > 0 && !selectedSize) {
      setToast({ id: Date.now().toString(), title: 'Attention', message: 'Veuillez sélectionner une taille', type: 'warning' });
      return;
    }
    
    if (product?.colors && product.colors.length > 0 && !selectedColor) {
      setToast({ id: Date.now().toString(), title: 'Attention', message: 'Veuillez sélectionner une couleur', type: 'warning' });
      return;
    }
    
    if (!product || product.stock === 0) {
      setToast({ id: Date.now().toString(), title: 'Erreur', message: 'Produit non disponible', type: 'error' });
      return;
    }
    ...
    setToast({ id: Date.now().toString(), title: 'Succès', message, type: 'success' });
  };
